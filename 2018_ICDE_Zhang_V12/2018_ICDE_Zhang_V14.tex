...
Third, the tree index $\treeindex$ contains at most $2\times 4^n$ nodes with the fields of ($l, nid, isLeaf$). Hence, the space cost of $\treeindex$ is $\bigoh(4^n)$.
...
\begin{algorithm}[!t]
	...
	\For{$T_q\in\{T_q\}$ \textbf{parallelly}}{
		...
		\textbf{//{~(1)~computing frequency distance by CPU}}\\
		\For{$T_i\in \alltraj$}{
...
For range query, we compare \frname with its CPU version, and two GPU approaches: STIG~\cite{7498315} and FSG~\cite{GPUTaxi}. These two GPU-based works are designed for spatial-temporal points rather than trajectories, so we add pointers between the points in the same trajectory to these works to allow trajectories to be recovered.
