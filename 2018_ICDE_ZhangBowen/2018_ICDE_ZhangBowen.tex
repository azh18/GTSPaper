...
\eat{
	...
	After this process, the complete-quadtree $QT$ is transformed into a PR-quadtree $\treeindex$. It can be easily seen that PR-quadtree satisfies the theorem~\ref{theo:Mnode}.
	
	\begin{theorem}\label{theo:Mnode}
		Let $LN$ be any leaf node of $\treeindex$, the total number of points within all the cells $c\in LN.block$ is less than $M_{node}$.
	\end{theorem}
	...
	\begin{definition}[findParent]\label{def:findParent}
		Let $N=(nid,level,isLeaf)$ be a node of PR-quadtree $\treeindex$ where $level>0$, $findParent$ is a map from the node $N$ to another node $N_p$, where $N_p.level=N.level-1$ and $N_p.nid=\lfloor N.nid/4\rfloor$.
	\end{definition}
	
	\begin{definition}[findDaughter]\label{def:findDaughter}
		Let $N=(nid,level,isLeaf)$ be a node of PR-quadtree $\treeindex$ where $isLeaf=FALSE$, $findDaughter$ is a map from the node $N$ to a set of node $N_D$, in which all $N_d\in N_D$ satisfy: $N_d.level=N.level+1$ and $N.nid=\lfloor N.d/4\rfloor$.
	\end{definition}
...
\noindent  {\bf PR-quadtree $\treeindex$~\cite{}}.
While cell-based trajectory table provides a way to organize trajectory data, it is easy to see that the number of points per cell may differ greatly. 
%Intuitively, cells in central areas of a city can have high point density and those in remote areas may contain zero point. 
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\ENSURE ~~\\
		Cell-based Trajectory Table $\trajtable$; PR-quadtree $\treeindex$;
		\FOR{$t\in \mathbb{D}$}
			\STATE $SEG \leftarrow splitTraj(t,\allcell)$
			\FOR{$s \in SEG$}
				\STATE $cid \leftarrow getCID(s)$
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		\REQUIRE ~~\\
		queryIDs $Q$; query ranges $R_Q$; dataset $\mathbb{D}$; PR-quadtree $\treeindex$, Cell-based Trajectory Table $\trajtable$; Memory Allocate Table $MAT$;
		\ENSURE ~~\\
		...
		\FOR{$q\in Q$ \textbf{parallelly}}
			...
			\STATE $C_N.insert(\treeindex.root)$
			\WHILE{$C_N\neq \phi$}
			...
			\ENDWHILE
		\ENDFOR
		\STATE $transferDatatoGPU(C_P,\trajtable,MAT)$
...
\eat{
We traverse the PR-quadtree $\treeindex$ from the root, and for each $N$ we access, we  to the layer $\lambda$ to find leaf nodes whose block intersect or overlap the MBR $mbr_R$ of $R$. This process can be finished by several $findDaughter$ operations. 
