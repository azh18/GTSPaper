...
The remainder of this paper is organized as follows. Section~\ref{sec:prelim} gives preliminaries. Section~\ref{sec:framework} describes our \frname framework. Section~\ref{sec:index} presents index structure. Section~\ref{sec:query} and Section~\ref{sec:optimization} introduce GPU-accelerated query processing approaches and optimizations, respectively. We provide experimental results in Section~\ref{sec:exp}, review related works in Section~\ref{sec:related} and conclude this paper in Section~\ref{sec:conclusion}.
...
Our index \idxname is thus developed based on a squared \emph{global grid}, covering all the trajectories. In our design, we simply partition the grid into $4^n$ cells of the same size along two geospatial coordinates, which can be visualized into a $2^n\times 2^n$ cell matrix (see Figure~\ref{}).
...
Based on a grid $\allcell$ of cells, we develop our spatial index \idxname following a two-part structure: cell-based trajectory table and PR-quadtree. Figure~\ref{} shows our index structure.
...
We illustrate an example of $\trajtable$ in Figure~\ref{}... [give an example?]
...
Such differences result in imbalanced workload on GPU during query processing (see details in Section~\ref{}). To alleviate the problem, we leverage PR-quadtree~\cite{} to group cells into coarse-grained \emph{block}s.
...
The first fraction is the cost of all data blocks, and four elements (longitude, latitude, timestamp and trajectory ID) need to be stored for a point. And it is also the cost of the raw trajectories. The second fraction represents the cost of cell-based trajectory table, because each node in it has three elements ($cid$, $offset$, $N_p$) and there are $\frac{L_t\times N_t}{M_{cell}}$ nodes. In Global Grid, only an pointer of data block need to be stored, so the cost is $4^\lambda$. At last, there are $\frac{4^\lambda -1}{3}$ nodes in complete PR-quadtree, and in each node $level$, $id$ and $isLeaf$ are neccessary to be retained, so the last fraction represents the cost of PR-quadtree.
...
Various spatial indices increase throughput by pruning a considerable amount of computation in the query execution. However, the computational cost of refinement still occupies a large propotion in the execution time of query processing even if there is an efficient indexing scheme. In range query, millions of points wait to be verified after the filter phase under the situation of large-scale queries. This phenomenon is more obvious for top-k similarity query processing, because the complexity of calculating EDR is quadratic. As we have mentioned in Section I, only 10 top-5 similarity query on a 9GB dataset spends about 700s, although a state-of-the-art pruning scheme is applied.
...
In this phase a set of query ranges $R_Q$ are input. Then for each query range $R_q$, a set of candidate points $Cand_q$ are transferred to global memory, waiting for verifying in the refinement phase.
...
\begin{figure}[!t]\centering
	\scriptsize{
		\begin{minipage}{0.48\linewidth}
			...
			(a) Example of Level 2 in PR-quadtree
		\end{minipage}
		\hfill
		\begin{minipage}{0.48\linewidth}
			\centering
			\includegraphics[width=\linewidth]{pdf/MBR_lv1.pdf}
			(b) Example of Level 1 in PR-quadtree
		\end{minipage}
	}
	\caption{An example of finding the cells overlapped or intersected given the query MBR under Morton-based encoding PR-quadtree and Global Grid\label{fig:MBR_FindCell}}
\end{figure}

We firstly denote ID of each query as $q$, forming a query set $Q$. then we traverse the PR-quadtree $QT$ from the root to the layer $\lambda$ to find leaf nodes which intersect or overlap the MBR $mbr_q$ of $R_q$. Under the Morton-based encoding, this process can be finished by several kinds of binary operations. 

Given $mbr_q$, we can judge in each layer which nodes overlap the MBR. Figure \ref{fig:MBR_FindCell}(a) shows the example of nodes in level 2 in a complete PR-quadtree. The coordinates of two vertexes are transformed to binary \{(00, 01), (10, 11)\}, then we can find that from the transformation shows at upper right, the ID of nodes containing two vertexes can be generated. Applying this transformation to the range of coordinates generated from MBR, as the table at lower right shows, we can generate all the nodes which the MBR overlap or intersect. Another example of nodes in level 1 is shown in figure \ref{fig:MBR_FindCell}(b). 
...
After generating the candidate nodes, we make the pair of data blocks and query's ID $(db_{q,i},q)$. For each candidate node $N_c$ in each query, the cells linked to $N_c$ are searched and corresponding datablocks are packed into a larger datablock $db_{q,i}$, then it is paired with $R_q$. Reminding that the number of points within a node is smaller than $M_{node}$, the tasks of verifying these pairs are in even scale. Finally, the query ranges $R_Q$, the information of pairs and the datablocks are transferred into global memory.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		\REQUIRE ~~\\
		query ranges $R_Q$; pairs $Cand_Q={pair(db,q)}$;
		\ENSURE ~~\\
		Result Trajectories $T_{result}$;
		\FOR{$cudaBlockID \in [0,size(Cand_Q)-1]$ \textbf{parallelly}}
			\STATE $blockDB \leftarrow Cand_Q[cudaBlockID].db$
			\STATE $queryID \leftarrow Cand_Q[cudaBlockID].q$
...
Given a set of query trajectories $Q_t$, the goal of the top-k similarity query is retrieve k closest trajectories measured by EDR for each $t_q\in Q_t$. The FDs between $t_q\in Q_t$ and trajectories $t_d\in \mathbb{D}$ are firstly calculated as the lower bound of EDR, then we iterate between generating candidates and calculating real EDRs in batch, until no candidates can be generated according to the FDs and the real EDRs. The algorithm is shown in Algorithm~\ref{alg:TSQ_1}.

\subsubsection{Frequency Distance Calculation}

In this component, we firstly use cell-based trajectory table $CT$ in \idxname to generate the frequency vectors (FV) of $t_q$ and $t_d$, which indicate the number of points within each cell for a trajectory. For a trajectory $t_i$ whose cell-based trajectory $L_i$ has $m$ nodes, we maintain an length-$m$ array $V_{freq}$ and initialize it to all-zero firstly. Then, we get the sequence of cells from $L_i$, and for each node $(cid,num,offset)$ we add $num$ to the cid-th value of $V_{freq}$. After processing the whole $L_i$, the array $V_{freq}$ becomes the FV of trajectory $t_i$. The FV of $t_q$ and $t_d$ can be generated by this method, and then the FD between them are calculated by CompHisDist algorithm\cite{DBLP:conf/sigmod/ChenOO05} with linear complexity.

\subsubsection{Candidate Generation}

Based on the FD between $t_q$ and all $t_d\in \mathbb{D}$, we generate the candidates which are likely to be the one of top-k closest trajectories of $t_q$ in every loop and then invoke GPU to calculate the real EDR between $t_q$ and them to reduce the times of complex EDR calculation.

We use a xxx scheme in the process of candidate generation to make sure that no trajectories which are probable to be one of the results are omitted. We firstly maintain a priority queue $FDPQ$ for each $t_q$ to store the FDs between $t_q$ and all trajectories of $\mathbb{D}$, with all calculated FDs together with the trajectory ID added into it. Then, in each iteration, different from traditional xx scheme which chooses one candidate once, we extract $l>1$ trajectories whose FDs are the smallest as a candidate set $Cand_q$ waiting for calculating real EDR, because in traditional situation the scale of candidates is small, while GPU can only get a remarkably high speedup under a large number of tasks. After $Cand_q$ is extracted, the trajectories in it are recovered by $CT$ and transferred into global memory of GPU, together with the information about $t_q$ and some parameters such as $\epsilon$ needed in the calculation of EDR. At the end of this iteration, for all $t_q$, the tasks of calculating EDR between each $t_d\in Cand_q$ and corrsponding $t_q$ are uniquely assigned to CUDA blocks. 
