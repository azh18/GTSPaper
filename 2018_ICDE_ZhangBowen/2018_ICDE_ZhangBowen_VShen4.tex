...
Our index \idxname is thus developed based on a squared \emph{global grid}, covering all the trajectories. In our design, we simply partition the grid into $4^\lambda$ cells of the same size along two geospatial coordinates, which can be visualized into a $2^\lambda \times 2^\lambda$ cell matrix (e.g., a grid of $16$ cells in Figure~\ref{fig:Index}).
...
\end{equation}
The above equation is guaranteed by the well-known relationship between Morton codes and quadtree structural properties~\cite{}.
More importantly, the cells in $\phi_c(N)$ resides continuously in the trajectory table $\trajtable$.



We construct $\treeindex$ by pruning $QT$ in a bottom-up way. That is, we recursively prune all child nodes of $N$ iff the total number of trajectory points covered by the child nodes is lower than a threshold $\theta_p$. The resulting tree is recognized as $\treeindex$ with a set of new leaf nodes, denoted by $\phi_l(\treeindex)$, that cannot be pruned further. For instance, in Figure~\ref{}, the new leaf nodes of $\treeindex$ are $\{...\}$. One can verify that new lead nodes in $\treeindex$ provide a grouping over fine-grained cells and each leaf is a block of cells covering at least $\theta_p$ trajectory points:
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		\REQUIRE ~~\\
		trajectory dataset $\mathbb{D}$; Query trajectory set $T_q=\{t_{q_i}|0\leq i< size(T_q)\}$; Global Grid $G$; Cell-based Trajectory Table $CT$; k value $k$; EDR parameter $\epsilon$;
		\ENSURE ~~\\
